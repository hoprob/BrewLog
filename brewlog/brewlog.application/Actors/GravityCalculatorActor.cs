...
namespace brewlog.application.Actors
{
    public class GravityCalculatorActor : ReceiveActor
    {
        ...
        {
            public record CalculatedAbvResponse(double Abv);
        ...
        public GravityCalculatorActor()
        {
            Receive<Queries.GetCalculatedAbv>(cmd =>
            {
                Sender.Tell(new Responses.CalculatedAbvResponse(
                    CalculateAbv(cmd.OriginalGravity, cmd.FinalGravity)));
            });
        ...
        private double CalculateFinalBoilVolume(double preBoilVolume, double boilMinutes)
        {
